...
var ReporteVehiculo = function () {
    ...
    var dataTableReporteVehiculoView = function (url) {

        //if (!$.fn.DataTable.isDataTable('#ReporteVehiculoDataTable')) {
        dataTableReporteVehiculo = $("#ReporteVehiculoDataTable").dataTable({
            "bFilter": false,
            "bProcessing": true,
            "serverSide": true,
            //"scrollY": "400px",
    ...
    var Eventos = function (ReporteVehiculo_CTL) {

        $(ReporteVehiculo_CTL).on("change", "select#TipoEmpresaSearch", function () {
        ...
        $(ReporteVehiculo_CTL).on("change", "select#IdEmpresaSearch", function () {
            dataTableReporteVehiculo.fnReloadAjax();
        });
        $(ReporteVehiculo_CTL).on("change", "select#IdTipoSearch", function () {
            dataTableReporteVehiculo.fnReloadAjax();
        });
        $(ReporteVehiculo_CTL).on("change", "select#IdMarcaSearch", function () {
            var id = $(this).val();
            webApp.ListaModelo(id);
            dataTableReporteVehiculo.fnReloadAjax();
        });
        $(ReporteVehiculo_CTL).on("change", "select#IdModeloSearch", function () {
            dataTableReporteVehiculo.fnReloadAjax();
        });
        $(ReporteVehiculo_CTL).on("change", "select#IdCategoriaSearch", function () {
            dataTableReporteVehiculo.fnReloadAjax();
        });
    ...
    return {
        init: function () {
            var _this = this;
            _this.ReporteVehiculo_CTL = $("#ReporteVehiculo");
            Eventos(_this.ReporteVehiculo_CTL);
            //Configuracion DataTable
            Utils.ConfigurarDataTable();
            checkSessionUser.checkSession(function () {
                dataTableReporteVehiculoView(urlReporteVehiculoListar);
            });
